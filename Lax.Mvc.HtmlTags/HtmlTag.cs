...
namespace Lax.Mvc.HtmlTags {
    ...
    public class HtmlTag : ITagSource, IHtmlContent {
        ...
        private static readonly HashSet<string> VoidElementTags =
        ...
        public static void UseMetadataSuffix(string suffix) => _metadataSuffix = suffix;
        ...
        public HtmlTag(string tag) : this() {
            _tag = tag;
            _ignoreClosingTag = VoidElementTags.Contains(_tag);
        }
        ...
        public void WriteTo(TextWriter writer, HtmlEncoder encoder) => WriteHtml(writer, encoder);
        ...
        private static HtmlTag WalkToTop(HtmlTag htmlTag) =>
            htmlTag.Parent == null ? htmlTag : WalkToTop(htmlTag.Parent);
        ...
        private void RenderAttribute(TextWriter html, HtmlEncoder encoder, string key, string value,
            bool attributeIsEncoded) {
            html.Write(" ");
            html.Write(key);
            html.Write("=\"");
            if (value != null) {
        ...
        private static bool IsCssClassAttr(string attribute) =>
            attribute.Equals(CssClassAttribute, StringComparison.OrdinalIgnoreCase);

        private static bool IsCssStyleAttr(string attribute) =>
            attribute.Equals(CssStyleAttribute, StringComparison.OrdinalIgnoreCase);

        private static bool IsMetadataAttr(string attribute) =>
            attribute.Equals(MetadataAttribute, StringComparison.OrdinalIgnoreCase);
            ...
            foreach (var parsedClass in classes) {
                if (!CssClassNameValidator.IsValidClassName(parsedClass)) {
                    throw new ArgumentException(
                        string.Format("CSS class names is not valid. Problem class was '{0}'", new[] {className}),
                        nameof(className));
                }
        ...
        public HtmlTag VisibleForRoles(IPrincipal principal, params string[] roles) =>
            Render(roles.Any(principal.IsInRole));
        ...
        public HtmlTag ForChild(string tagName) =>
            ((ITagSource) this).AllTags().First(child => child.TagName().EqualsIgnoreCase(tagName));
