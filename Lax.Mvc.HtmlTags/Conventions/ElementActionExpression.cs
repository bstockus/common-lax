...
namespace Lax.Mvc.HtmlTags.Conventions {
    ...
    public class ElementActionExpression {
        ...
        public void BuildBy<T>() where T : IElementBuilder, new() => BuildBy(new T());
        ...
        public void ModifyWith<T>() where T : IElementModifier, new() => ModifyWith(new T());
        ...
        public void Attr(string attName, object value) =>
            ModifyWith(req => req.CurrentTag.Attr(attName, value),
                string.Format("Set @{0} = '{1}'", new[] {attName, value}));

        public void AddClass(string className) => ModifyWith(req => req.CurrentTag.AddClass(className),
