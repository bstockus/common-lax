...
namespace Lax.Mvc.AdminLte.Bootstrap.Extensions {
    ...
    public static class TagHelperExtensions {
        ...
        public static async Task<TagHelperContent> ToTagHelperContentAsync(this ITagHelper tagHelper) =>
            await ToTagHelperContentAsync(tagHelper, (TagHelperContent) null);

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this ITagHelper tagHelper, Options options) {
            if (options.Context == null) {
                options.Context = new TagHelperContext(new TagHelperAttributeList(), new Dictionary<object, object>(),
            ...
            var output = new TagHelperOutput(options.TagName, new TagHelperAttributeList(options.Attributes),
                (b, e) => Task<TagHelperContent>.Factory.StartNew(() => options.Content)) {TagMode = options.TagMode};
        ...
        public static async Task<TagHelperContent> ToTagHelperContentAsync(this ITagHelper tagHelper,
            bool initTagHelper) =>
            await ToTagHelperContentAsync(tagHelper, new Options {InitTagHelper = initTagHelper});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this ITagHelper tagHelper,
            TagHelperContent content) =>
            await ToTagHelperContentAsync(tagHelper, new Options {Content = content});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this ITagHelper tagHelper, string tagName) =>
            await ToTagHelperContentAsync(tagHelper, new Options {TagName = tagName});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this ITagHelper tagHelper,
            TagHelperContext context) =>
            await ToTagHelperContentAsync(tagHelper, new Options {Context = context});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this ITagHelper tagHelper,
            IEnumerable<TagHelperAttribute> attributes) =>
            await ToTagHelperContentAsync(tagHelper, new Options {Attributes = attributes});
        ...
        public static async Task RunTagHelperAsync(this ITagHelper tagHelper, Options options) {
            ...
            var output = new TagHelperOutput(options.TagName, new TagHelperAttributeList(options.Attributes),
                (b, e) => Task.Factory.StartNew(() => options.Content)) {TagMode = options.TagMode};
            if (options.InitTagHelper) {
                tagHelper.Init(options.Context);
            }
        ...
        public static string GetTagName(this ITagHelper tagHelper) =>
            tagHelper.GetType().GetTypeInfo().GetCustomAttributes<HtmlTargetElementAttribute>()
                .FirstOrDefault(a => a.Tag != "*")?.Tag
            ?? Regex.Replace(tagHelper.GetType().Name.Replace("TagHelper", ""), "([A-Z])", "-$1").Trim('-')
                .ToLower();
        ...
        public static string GetTagName(this IEnumerable<ITagHelper> tagHelpers) {
            ...
            return
                tagHelperList.OrderBy(tH => tH.Order)
                    ...
                    .FirstOrDefault(a => a.Tag != "*")?.Tag
                ?? Regex.Replace(tagHelperList.Min(tH => tH.Order).GetType().Name.Replace("TagHelper", ""), "([A-Z])",
                    "-$1").Trim('-').ToLower();
        }

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this IEnumerable<ITagHelper> tagHelpers) =>
            await ToTagHelperContentAsync(tagHelpers, new Options());

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this IEnumerable<ITagHelper> tagHelpers,
            TagHelperContent content) =>
            await ToTagHelperContentAsync(tagHelpers, new Options {Content = content});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this IEnumerable<ITagHelper> tagHelpers,
            string tagName) =>
            await ToTagHelperContentAsync(tagHelpers, new Options {TagName = tagName});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this IEnumerable<ITagHelper> tagHelpers,
            TagHelperContext context) =>
            await ToTagHelperContentAsync(tagHelpers, new Options {Context = context});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this IEnumerable<ITagHelper> tagHelpers,
            IEnumerable<TagHelperAttribute> attributes) =>
            await ToTagHelperContentAsync(tagHelpers, new Options {Attributes = attributes});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this IEnumerable<ITagHelper> tagHelpers,
            bool initTagHelper) =>
            await ToTagHelperContentAsync(tagHelpers, new Options {InitTagHelper = initTagHelper});

        public static async Task<TagHelperContent> ToTagHelperContentAsync(this IEnumerable<ITagHelper> tagHelpers,
            Options options) {
            var helpers = tagHelpers.ToList();
            var tagHelperList = tagHelpers as List<ITagHelper> ?? helpers.ToList();
            if (options.Context == null) {
                options.Context = new TagHelperContext(new TagHelperAttributeList(), new Dictionary<object, object>(),
                    Guid.NewGuid().ToString("N"));
            }
            ...
            if (options.TagName == null) {
                options.TagName = helpers.GetTagName();
            }
